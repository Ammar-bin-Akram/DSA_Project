...
def plot_weighted_edges_and_path_with_delay(weighted_edges, path, delay=0.15):
    fig = plt.figure()
    ...
    if path:
        ...
        for i in range(len(path_coords[0])):
            ax.plot(path_coords[0][:i + 1], path_coords[1][:i + 1], path_coords[2][:i + 1], c='green', marker='o',
                    linestyle='-')
            plt.pause(delay)
...
def plot_3d_path(path, obstacles):
    ...
    ax.scatter(obstacle_coords[0], obstacle_coords[1], obstacle_coords[2], c='red', marker='X')
    ...
    if path:
        path_coords = list(zip(*path))
        ax.plot(path_coords[0], path_coords[1], path_coords[2], c='green', marker='o', linestyle='-')
...
if shortest_path is not None:
    ...
    plot_weighted_edges_and_path_with_delay(weighted_edges, shortest_path, delay=0.15)
else:
