...
def plot_3d_grid_with_delay(grid, delay):
    fig = plt.figure()
...
def plot_a_star_path_on_grid(path, grid, start, goal, delay):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection="3d")
...
# starttime = time.time()
path, totaltime = a_star_3d(start, goal, grid)
# endtime = time.time()
...
if path is not None:
    ...
    plot_a_star_path_on_grid(path, grid, start, goal, delay=0.000001)
